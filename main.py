...
def curvecalc():
    ...
    global heatingcurve
    global saved_settemp_before_direct, last_heatingcurve_mode, last_direct_settemp_value, direct_exit_guard_until
    insidetemp = None
    outsidetemp = None
    settemp = None
    heatcurve = None
    ...
    if heatingcurve != 'directly' and last_heatingcurve_mode == 'directly' and saved_settemp_before_direct is not None:
        try:
            ...
            # Wepchnij od razu prawidłową wartość do UI (żeby nie została stara z Direct)
            try:
                socketlocal.emit('data_update', {'setpoint': float(saved_settemp_before_direct)})
                if remote:
                    sio_emit = {'data_update': {'setpoint': float(saved_settemp_before_direct)}}
                    sio_remote.emit('data_from_device', sio_emit)
            except Exception:
                logging.debug('Nie udało się wysłać setpoint do UI po wyjściu z Direct')
            # Krótka blokada na spóźnione eventy UI po przełączeniu z Direct -> manual/auto
            direct_exit_guard_until = time.time() + 3.0
        except Exception:
    ...
    try:
        ...
        if _threeway == 'DEFROST' or _defrost == 'on':
            ...
            # Zapamiętaj aktualny tryb, żeby nie mylić \"wejścia\" w Direct po defroście
            last_heatingcurve_mode = str(heatingcurve).strip().lower()
    ...
    if heatingcurve == 'directly':
        ...
        if last_heatingcurve_mode != 'directly':
            ...
            if saved_settemp_before_direct is None and _st_prev is not None and isfloat(_st_prev):
            ...
            try:
                ...
                if _hc_prev is not None and isfloat(_hc_prev):
                    ...
                    settemp = _hc_prev
                    # W Direct traktujemy settemp jako zadaną CO (dla UI i logiki), więc zapamiętaj wartość
                    last_direct_settemp_value = _hc_prev
                    direct_exit_guard_until = 0.0
                    # Wepchnij od razu do UI (żeby zadana CO i temp CO były spójne)
                    try:
                        socketlocal.emit('data_update', {'setpoint': _hc_prev})
                        if remote:
                            sio_emit = {'data_update': {'setpoint': _hc_prev}}
                            sio_remote.emit('data_from_device', sio_emit)
                    except Exception:
                        logging.debug('Nie udało się wysłać setpoint do UI po wejściu w Direct')
...
def change_temp_route():
    global last_direct_settemp_value, direct_exit_guard_until
    which = request.form['which']
    value = request.form['value']
    directly = request.form['directly']

    # Ochrona: tuż po wyjściu z Direct UI może wysłać starą wartość z Direct jako settemp (directly=0).
    if which == 'heat' and str(directly) == '0':
        try:
            if time.time() < float(direct_exit_guard_until) and last_direct_settemp_value is not None and isfloat(value):
                if abs(float(value) - float(last_direct_settemp_value)) < 0.05:
                    cur = statusdict.get('settemp', {}).get('value', value)
                    socketlocal.emit('data_update', {'setpoint': cur})
                    return jsonify(msg='OK', state='success')
        except Exception:
            pass

    msg, response = new_tempchange(which, value, directly)

    # Direct: gdy ustawiamy bezpośrednio (directly=1), odśwież hcurve natychmiast
    if which == 'heat' and str(directly) == '1' and str(heatingcurve).strip().lower() == 'directly':
        try:
            v = float(value)
            last_direct_settemp_value = v
            saveconfig('SETTINGS', 'settemp', v)
            ischanged('hcurve', v)
        except Exception:
            logging.exception('Direct: failed to sync hcurve/settemp after tempchange')

    # Direct: natychmiastowa aktualizacja po zmianie settemp (directly=0) – bez czekania na cykl curvecalc
...
#        emit('return', {'info': 'ok', 'status': 'success'})
    ...
    if 'tempchange' in data:
        # Ochrona: tuż po wyjściu z Direct UI czasem wysyła jeszcze starą wartość z Direct jako settemp (directly=0).
        if data.get('tempchange') == 'heat' and str(data.get('directly')) == "0":
            try:
                if time.time() < float(direct_exit_guard_until) and last_direct_settemp_value is not None and isfloat(data.get('value')):
                    if abs(float(data['value']) - float(last_direct_settemp_value)) < 0.05:
                        logging.info(f"IGNORED stale settemp after Direct exit: {data['value']}")
                        cur = statusdict.get('settemp', {}).get('value', data['value'])
                        emit('data_update', {'setpoint': cur})
                        emit('return', {'tempchange': 'OK', 'type': 'success'})
                        return
            except Exception:
                pass
        ...
        # Direct: gdy ustawiamy bezpośrednio (directly=1), odśwież hcurve natychmiast (bez czekania na curvecalc)
        if data.get('tempchange') == 'heat' and str(data.get('directly')) == "1" and str(heatingcurve).strip().lower() == 'directly':
            try:
                v = float(data['value'])
                last_direct_settemp_value = v
                # Trzymaj w config/statusdict bieżącą wartość Direct (dla spójności UI + logiki)
                saveconfig('SETTINGS', 'settemp', v)
                ischanged('hcurve', v)
            except Exception:
                logging.exception('Direct: failed to sync hcurve/settemp after direct tempchange')
        ...
        try:
            ...
            try:
                ...
                if _new_heatingcurve != _old_heatingcurve:
                    ...
                    # Jeśli wychodzimy z Direct, przywróć settemp sprzed Direct zanim UI zdąży je nadpisać
                    if _old_heatingcurve == 'directly' and _new_heatingcurve != 'directly' and saved_settemp_before_direct is not None:
                        try:
                            logging.info(f"EXIT DIRECT (settings): restore settemp={saved_settemp_before_direct}")
                            new_tempchange('heat', float(saved_settemp_before_direct), '0')
                            emit("data_update", {'setpoint': float(saved_settemp_before_direct)})
                            direct_exit_guard_until = time.time() + 3.0
                        except Exception:
                            logging.exception("EXIT DIRECT (settings): restore failed")
                        saved_settemp_before_direct = None
                        last_heatingcurve_mode = _new_heatingcurve
                    curvecalc()
            except Exception:
