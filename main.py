...
'tempcompensation':{'mqtt':'/details/tempcompensation/state','value':'N.A.'},
'tempcompensation_set':{'mqtt':'/details/tempcompensation_set/state','value':'N.A.'},
...
'error':{'mqtt':'/details/error/state','value':'N.A'},
'lasterror':{'mqtt':'/details/lasterror/state','value':'N.A'},
...
def on_message(client, userdata, msg):  # The callback for when a PUBLISH 
    ...
    elif msg.topic == topic + "/tempcompensation/set":
        try:
            mesg, response = new_tempcompchange(msg.payload.decode('utf-8'))
            if response:
                client.publish(topic + "/details/tempcompensation_set/state", str(statusdict.get("tempcompensation_set", {}).get("value", "N.A.")), qos=1, retain=True)
        except Exception:
...
def new_tempcompchange(value):
    ...
    - value: float/string, normalized to 0.5°C steps
    """
    global R101
    raw = str(value).strip().replace(",", ".")
    try:
    ...
    # IMPORTANT: PyHaier SetTempCompensation (A03) expects "half-degree steps" as int
    # (-30..30) in most builds. Passing float can silently return "ERROR" (string),
    # which then never reaches the pump.
    steps = int(round(v * 2.0))  # -30..30
    frame = PyHaier.SetTempCompensation(R101, steps)

    # Some implementations return "ERROR" instead of raising; guard for that.
    if not isinstance(frame, list) or len(frame) != 6:
        logging.error("Temp compensation set: bad frame from library: %s", frame)
        return gettext("Temp compensation"), False

    response = set_newframe(R101, frame)
    if response:
        # Store last requested value for UI/HA
...
def curvecalc():
    ...
    if heatingcurve == 'directly':
        ...
        try:
            ...
            if outsidetemp is not None and zone == 'frost':
                ...
                # If user selected Quiet (without FLimit), ensure relay stays OFF
                try:
                    if not flimit_allowed:
                        _flimiton = str(statusdict.get('flimiton', {}).get('value', '0')).strip()
                        if _flimiton == '1':
                            logging.info('Direct: TempZone(frost): forcing OFF frequency limit relay (Quiet-only)')
                            flimitchange('0')
                except Exception as e:
                    logging.warning(f"Direct: TempZone(frost): cannot control flimit relay: {e}")

            else:
                # TempZone warm should also work in Direct (Turbo/Eco/Quiet/Quiet+FLimit)
                allow_auto_flimit = True

                if outsidetemp is not None and zone == 'warm':
                    try:
                        ...
                        # Quiet-only: ensure FLimit relay stays OFF and block auto-flimit
                        if not flimit_allowed:
                            allow_auto_flimit = False
                            _flimiton = str(statusdict.get('flimiton', {}).get('value', '0')).strip()
                            if _flimiton == '1':
                                logging.info('Direct: TempZone(warm): forcing OFF frequency limit relay (Quiet-only)')
                                flimitchange('0')
                    except Exception as e:
                        logging.warning(f"Direct: TempZone(warm): cannot apply zone mode: {e}")

                if allow_auto_flimit and flimit == "auto" and antionoff != "1" and outsidetemp is not None:
    ...
    if zone == 'frost':
        ...
        # If user selected Quiet (without FLimit), ensure relay is OFF
        try:
            if not flimit_allowed:
                _flimiton = str(statusdict.get('flimiton', {}).get('value', '0'))
                if _flimiton == '1':
                    logging.info('TempZone(frost): forcing OFF frequency limit relay (Quiet-only)')
                    flimitchange('0')
        except Exception as e:
        ...
        # Optional: allow outside-temp based FLimit auto in frost zone unless disabled by Quiet-only
        try:
            if flimit_allowed and flimit == "auto" and antionoff != "1" and outsidetemp is not None:
                threeway = statusdict.get('threeway', {}).get('value', '')
                compinfo = statusdict.get('compinfo', {}).get('value', [])
                if isinstance(compinfo, list) and len(compinfo) > 0:
                    if not (dhwwl == "1" and compinfo[0] != 0 and str(threeway).strip().upper() == "DHW"):
                        if outsidetemp >= float(flimittemp):
                            logging.info("TempZone(frost): Turn on freq limit")
                            flimitchange("1")
                        elif outsidetemp <= float(flimittemp) + 0.5:
                            logging.info("TempZone(frost): Turn off freq limit")
                            flimitchange("0")
        except Exception:
            logging.exception('TempZone(frost): flimit auto-change failed')
    ...
    if zone == 'warm':
        try:
            ...
            if not flimit_allowed:
                _flimiton = str(statusdict.get('flimiton', {}).get('value', '0'))
                if _flimiton == '1':
                    logging.info('TempZone(warm): forcing OFF frequency limit relay (Quiet-only)')
                    flimitchange('0')
        except Exception as e:
    ...
    if len(compinfo) > 0:
        if dhwwl=="1" and compinfo[0] != 0 and threeway == "DHW":
            ...
            if tempzone == 'warm':
                try:
                    flimit_allowed_here = (normalize_zone_mode(zone_warm_mode, 'quiet_flimit') != 'quiet')
                except Exception:
                    flimit_allowed_here = True

            if flimit_allowed_here and flimit == "auto" and antionoff != "1" and tempzone != 'frost':
                if outsidetemp >= float(flimittemp):
...
def getdata():
    ...
    # NOWE - Error, LastError, TempCompenstation
...
def log_status_snapshot(interval_s=30):
    ...
    try:
        exclude_snapshot_keys = {"chkwhpd", "dhwkwhpd", "theme", "tempcompensation_set"}
...
def configure_ha_mqtt_discovery():
    ...
    def configure_number(name, command_topic, status_topic, unique_id, unit, min, max, device_class, step="0.1"):
        msg = json.dumps(
            {
                "name" : name,
                "cmd_t" : command_topic,
    ...
    configure_sensor("Temp compensation (A03)", mqtt_topic + "/details/tempcompensation/state", "HaierPi_TempComp", "°C", "temperature", "measurement", None)
    configure_number("Temp compensation set (A03)", mqtt_topic + "/tempcompensation/set", mqtt_topic + "/details/tempcompensation_set/state", "HaierPi_TempCompSet", "°C", -15.0, 15.0, "temperature", step="0.5")
