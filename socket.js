...
let lastPch = "off";
let lastPdhw = "off";
// ---- Δt visibility / rendering (single source of truth) ---------------------
// Requirements (as requested):
// - In zones warm/frost: hide Δt (show zone icon instead)
// - In normal: show Δt only if Anti On-Off = 1 AND compressor running AND threeway == CH
// - In defrost: hide
// - If Δt is not numeric: hide (do not show "N.A.")
// This avoids flicker caused by transient N.A values from backend updates.

let __aood_lastDeltaNum = null;
let __aood_lastCompHz = 0;
let __aood_lastThreeway = "";
let __aood_lastDefrost = "off";
let __aood_lastAntionoff = "0";
let __aood_lastTempZone = (window.__tempZone || "normal");

function __aood_isDefrostOn(v) {
  const s = String(v ?? "").toLowerCase();
  return s === "on" || s === "1" || s === "true";
}

function __aood_parseNumber(v) {
...
function renderDelta() {
  const wrap = document.getElementById("aood-delta-wrap");
  const deltaEl = document.getElementById("delta");
  if (!wrap || !deltaEl) return;

  const tz = String(__aood_lastTempZone || window.__tempZone || "normal").toLowerCase();
  const inZone = (tz === "warm" || tz === "frost");
  const antiOn = String(__aood_lastAntionoff) === "1";
  const defrostOn = __aood_isDefrostOn(__aood_lastDefrost);
  const compRunning = Number(__aood_lastCompHz) > 0;
  const tw = String(__aood_lastThreeway || "").toUpperCase();
  const isCH = (tw === "CH"); // backend uses "CH" (UI may show "CO" elsewhere)
  const hasDelta = (typeof __aood_lastDeltaNum === "number") && Number.isFinite(__aood_lastDeltaNum);

  const shouldShow = (!inZone) && antiOn && (!defrostOn) && compRunning && isCH && hasDelta;

  if (shouldShow) {
    deltaEl.textContent = __aood_lastDeltaNum.toFixed(1);
    wrap.style.display = "";
  } else {
    // Keep text untouched, but do not show N.A (wrap hidden)
    wrap.style.display = "none";
  }
}

// Backward-compat (old code paths may call this)
function updateDeltaVisibility() { renderDelta(); }
// -----------------------------------------------------------------------------
...
if (path != '/settings') {
    ...
    break;
	    	...
	    	case "compinfo":
				...
				__aood_lastCompHz = __aood_parseNumber(msg.compinfo && msg.compinfo.length ? msg.compinfo[0] : null) || 0;
				renderDelta();
            ...
            case "threeway":
                // 3-way valve status (CH/DHW only)
                {
                    ...
                    // Store raw for Δt visibility (backend uses CH/DHW)
                    __aood_lastThreeway = (twUp.includes("DHW") || twUp.includes("CWU")) ? "DHW" : ((twUp.includes("CH") || twUp.includes("CO") || twUp.includes("HEAT")) ? "CH" : __aood_lastThreeway);
                    renderDelta();
...
case "archerror":
	    	...
	    	case "aoodt":
				...
				break;
	    	case "delta":
                // Δt from backend (may be numeric or "N.A.")
                __aood_lastDeltaNum = __aood_parseNumber(msg.delta);
                renderDelta();
                break;
		    case "flimiton":
...
case "outtemp":
			...
			case "tempzone":
                ...
                __aood_lastTempZone = window.__tempZone;
                renderDelta();
                break;
	    	case "outtemp_status":
...
case "humid":
		    ...
		    case "ltemp":
				...
				break;
	    	case "antionoff":
                __aood_lastAntionoff = String(msg.antionoff ?? "");
                if(msg.antionoff == "1") {
                    $("#presetdiv").hide();
                    $("#antionoffdiv").show();
                } else {
