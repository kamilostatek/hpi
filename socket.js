...
window.__antiOnOff = window.__antiOnOff || 0;  // 1 when Anty ON-OFF is enabled
window.__defrostOn = window.__defrostOn || 0;   // 1 when defrost is active
...
function updateDeltaVisibility() {
  ...
  if (!deltaEl.length) return; // element exists only on main dashboard (antionoff card)
    const wrap = $("#aood-delta-wrap"); // wrapper for Δt on dashboard

  // Respect temperature zones: when zone icon is shown, hide the Δt span
  const zoneWrapEl = document.getElementById('aood-zone-wrap');
  if (zoneWrapEl) {
    try {
      const disp = window.getComputedStyle(zoneWrapEl).display;
      if (disp && disp !== 'none') {
  ...
  const hasDelta = Number.isFinite(deltaNum);
    const antiOn = (window.__antiOnOff === 1) || ($("#antionoffdiv").length && $("#antionoffdiv").is(":visible"));
  const defrostOn = (window.__defrostOn === 1) || ($("#defrost").length && $("#defrost").hasClass("defrost-pulse"));
  const shouldShow = hasDelta && antiOn && (compHz > 0) && (tw === "CH") && !defrostOn;
...
case "humid":
	    	...
	    	case "antionoff":
				if(msg.antionoff == "1") {
				window.__antiOnOff = (String(msg.antionoff) === "1") ? 1 : 0;
				    $("#presetdiv").hide();
				    $("#antionoffdiv").show();
				}
				window.__antiOnOff = (String(msg.antionoff) === "1") ? 1 : 0;
