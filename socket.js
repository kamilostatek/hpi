document.addEventListener("DOMContentLoaded", function() {
	...
	// Reset placeholders on reconnect/disconnect to avoid stale values after backend restart
	socket.on('connect', function() {
		// Temps
		lastIntempValue = null;
		lastIntempSrc = null;
		renderIntemp(adds);
		// Humidity (show N.A until first update)
		if ($("#humid").length) $("#humid").text("N.A %");
	});
	socket.on('disconnect', function() {
		// Humidity should not keep old value when backend stops/restarts
		if ($("#humid").length) $("#humid").text("N.A %");
	});
...
function renderIntemp(adds) {
  ...
  // Emergency source: keep original behavior
  if (src === "emergency") {
