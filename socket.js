...
function updateDeltaVisibility() {
  ...
  const deltaEl = $("#delta");
  if (!wrap.length || !deltaEl.length) return;

  // Read current state
  const tz = String(window.__tempZone || "normal").toLowerCase();
  const def = String(window.__defrost || "off").toLowerCase();
  const anti = String(window.__antionoff || "0");
  const compHz = (typeof compinfo !== "undefined" && compinfo && compinfo.length) ? Number(compinfo[0]) : 0;
  const tw = (lastThreeway !== null && lastThreeway !== undefined) ? String(lastThreeway).toUpperCase() : "";

  // Parse delta number (accept comma)
  const raw = String(deltaEl.text() || "").trim();
  const dn = Number(raw.replace(",", "."));
  const hasDelta = Number.isFinite(dn);

  const inZone = (tz === "warm" || tz === "frost");
  const inDefrost = (def === "on" || def === "1" || def === "true");
  const antiOn = (anti === "1" || anti === "on" || anti === "true");
  const onCH = (tw === "CH"); // backend should send CH; keep strict
  const compressorRunning = (compHz > 0);

  const shouldShow = (!inZone) && (!inDefrost) && antiOn && compressorRunning && onCH && hasDelta;

  // Never show placeholder; hide when not eligible.
  wrap.css("display", shouldShow ? "" : "none");
  if (shouldShow) {
    // Normalize formatting (1 decimal) without changing meaning
    deltaEl.text(dn.toFixed(1));
  }
}
...
if (path != '/settings') {
    ...
    break;
	    	...
	    	case "heater":
				...
				break;
			case "defrost":
				window.__defrost = String(msg.defrost);
				// Defrost:
...
case "humid":
		    ...
		    case "ltemp":
				...
				break;
	    	case "antionoff":
				window.__antionoff = String(msg.antionoff);
				if(msg.antionoff == "1") {
				    $("#presetdiv").hide();
				    $("#antionoffdiv").show();
				}
