...
function updateDeltaVisibility() {
  ...
  let wrap = $("#aood-delta-wrap");
  if (!wrap.length) {
    // Fallback for older markup
    wrap = deltaEl.closest("span");
    if (wrap.length && wrap[0] === deltaEl[0]) wrap = deltaEl.parent();
  }
  if (!wrap.length) return;

  // Hide Δt in temperature zones (UI shows icon instead)
  ...
  // Hide Δt during defrost (if we know the flag)
  const def = (typeof lastDefrost !== "undefined" && lastDefrost !== null) ? String(lastDefrost).toLowerCase() : "off";
  if (def === "on" || def === "true" || def === "1") {
    wrap.hide();
    return;
  }

  const antiOnOffOn = (typeof lastAntiOnOff !== "undefined" && lastAntiOnOff !== null) ? String(lastAntiOnOff) === "1" : true;
  ...
  // Show only when: anti-onoff enabled + compressor running + threeway on CH + delta numeric
  const isCH = (tw === "CH");
  const shouldShow = antiOnOffOn && hasDelta && (compHz > 0) && isCH;
...
case "archerror":
	    	...
	    	case "aoodt":
				...
				break;
	    	case "delta":
				// Keep last numeric Δt to avoid flicker when backend briefly sends "N.A."
    const _raw = (msg && msg.delta !== undefined && msg.delta !== null) ? String(msg.delta).trim() : "";
    const _num = Number(_raw.replace(',', '.'));
    if (Number.isFinite(_num)) {
      window.__lastDeltaGood = _num;
      $("#delta").text(_num.toFixed(1));
    } else if (window.__lastDeltaGood !== undefined && window.__lastDeltaGood !== null) {
      // Do not overwrite a valid value with "N.A." while running
      $("#delta").text(Number(window.__lastDeltaGood).toFixed(1));
    } else {
      $("#delta").text(_raw || "N.A.");
    }
    updateDeltaVisibility();
