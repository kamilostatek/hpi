...
// --- Robust send helper: Socket.IO when connected, otherwise HTTP POST fallback ---
function sendTempChange(which, temp, direct) {
  // 1) Socket.IO (preferred)
  try {
    const s = window.socket || (typeof socket !== 'undefined' ? socket : null);
    if (s && typeof s.emit === 'function') {
      // If not connected, fall back to HTTP (otherwise user sees "changes only after refresh")
      if (s.connected) {
        s.emit("client", { tempchange: which, value: temp, directly: direct });
        return;
      }
    }
  } catch (e) {
  ...
  // 2) HTTP fallback (works even if websocket breaks)
  try {
    if (typeof $ !== 'undefined' && typeof $.post === 'function') {
      $.post('/tempchange', { which: which, value: temp, directly: direct }, function (data) {
        try {
          const text = data?.msg ?? data?.message ?? data;
          const rawState = data?.state ?? data?.type ?? 'info';
          const state = (rawState === 'error') ? 'danger' : rawState;
          if (typeof appendAlert === 'function') appendAlert(text, state);
        } catch (e) {}
      });
    } else {
      console.warn("sendTempChange: jQuery $.post not available; cannot send /tempchange");
    }
  } catch (e) {
    console.warn("sendTempChange failed:", e);
  }
}
